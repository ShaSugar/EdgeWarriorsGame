...
public class PGL_Main : MonoBehaviour
{
    ...
    private static Dictionary<int, PGL_MonsterConfigData.PGL_MonsterConfig> monsterConfigs =
        new Dictionary<int, PGL_MonsterConfigData.PGL_MonsterConfig>();

    private static Dictionary<int, PGL_MonsterConfigData.PGL_MonsterGroupConfig> monsterGroupConfigs =
        new Dictionary<int, PGL_MonsterConfigData.PGL_MonsterGroupConfig>();

    public static IEnumerator PreLoadResource()
    {
        ...
        string configsPath = $"Config/PlotGamePlay/MonsterConfigData_{curSceneName}";
        ResourceRequest configsRequest =
        ...
        if (monsterConfigsData != null && monsterConfigsData.groupData != null &&
            monsterConfigsData.groupData.Count > 0)
        {
    ...
    [SerializeField] private Transform _mainCameraRootTran, _mainCameraSubRootTran;
    [SerializeField] private Animation _mainCameraRootAnim, _mainCameraSubRootAnim;
    [SerializeField] private PGL_CameraPathsController _cameraPathsController;
    ...
    private void Awake()
    {
        ...
        MainCameraRootTran = (_mainCameraRootTran ? _mainCameraRootTran : CameraController.Instance.MainCameraRootTran);
        MainCameraRootAnim = (_mainCameraRootAnim ? _mainCameraRootAnim : CameraController.Instance.MainCameraRootAnim);
        MainCameraSubRootTran = (_mainCameraSubRootTran
            ? _mainCameraSubRootTran
            : CameraController.Instance.MainCameraSubRootTran);
        MainCameraSubRootAnim = (_mainCameraSubRootAnim
        ...
        // if (cameraPathAnims is not { Length: > 0 })
        // {
        //     Debug.LogError($"剧情场景没有配置相机动画呢！！！！");
        //     return;
        // }
        // else
        // {
        //     for (var i = 0; i < cameraPathAnims.Length; i++)
        //     {
        //         cameraPathAnims[i].animationObject = MainCameraRootTran;
        //         cameraPathAnims[i].playOnStart = false;
        //         cameraPathAnims[i].Stop();
        //     }
        //
        //     curCameraPathAnim = cameraPathAnims[0];
        // }
    }
    ...
    private void Start()
    {
        ...
        _cameraPathsController.GameStart(MainCameraRootTran);
        // StartLevel();
    }
    ...
    private void OnCountDownFinished(string s, object o)
    {
        ...
        EventMgr.Instance.Emit(LevelContinueTips_UICtrl.ShowTipsEvent,
            GameSceneMgr.Instance.CurSceneContinueTime()); //触发 显示是否继续
    }
